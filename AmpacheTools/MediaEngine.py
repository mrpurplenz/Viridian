...
class AudioEngine:
	...
	def __init__(self, ampache_conn):
		"""To construct an AudioEngine object you need to pass it an PimpacheSession object."""
		##################################
		# Variables
		##################################
	...
	def play_from_list_of_songs(self, songs_list, song_num=0):
		...
		This function will use the PimpacheSession to turn song_ids into song_urls."""
		if not songs_list:
	...
	def play_from_list_of_videos(self, videos_list, video_num=0):
		"""Takes a list of video_ids and position in the list and plays it.
		This function will use the PimpacheSession to turn video_ids into video_urls."""
		if not videos_list:
			print "Can't play empty list"
			return False
		self.videos_list = videos_list
		self.video_num = video_num
		try: # get the video_url and play it
			video_url = self.ampache_conn.get_video_url( self.videos_list[self.video_num] )
			self.player.set_state(gst.STATE_NULL)
			self.player.set_property('uri', video_url)
			self.player.set_state(gst.STATE_PLAYING)
			if self.ampache_gui != None:
				self.ampache_gui.mediaengine_video_changed(videos_list[video_num]) # hook into GUI
		except: # out of songs
			self.stop()
			if self.ampache_gui != None:
				self.ampache_gui.mediaengine_video_changed(None) # hook into GUI
			print "No more songs"
