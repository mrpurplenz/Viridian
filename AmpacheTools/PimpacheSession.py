...
class PimpacheSession:
	"""
	The PimpacheSession Class.  This is used to communicate to Ampache via the API.
	"""
	def __init__(self):
		"""
		Initialize an PimpacheSession.
		"""
		self.url = None
		self.username = None
		self.password = None
	...
	def has_credentials(self):
		"""
		Checks to see if the PimpacheSession object has credentials set.
		"""
	...
	def get_video_url(self, video_id):
		"""
		Takes a video_id and returns the url to the song (with the current authentication).
		"""
		values = {'action' : 'video',
			  'filter' : video_id,
			  'auth'   : self.auth,
		...
		video_url = video.getElementsByTagName('url')[0].childNodes[0].data
		return video_url
	...
	def get_videos(self, offset=None):
		"""
		Gets all videos and returns as a list of dictionaries.
		Example: [
		  		{	'video_id' 	 : video_id,
		...
		"""
		if offset == None:
			if self.videos_num > 100: # offset needed
				print "over 5000"
				list = []
				for i in range(0, self.videos_num, 100):
					list += self.get_videos(i)
				return list
			values = {'action' : 'videos',
				  'auth'   : self.auth,
			}
		else:
			values = {'action' : 'videos',
		...
		try:
			for video in nodes:
				video_id       = int(video.getAttribute('id'))
				video_title    = video.getElementsByTagName('title')[0].childNodes[0].data
				mime           = video.getElementsByTagName('mime')[0].childNodes[0].data
				resolution     = video.getElementsByTagName('resolution')[0].childNodes[0].data
				size           = int(video.getElementsByTagName('size')[0].childNodes[0].data)
				...
				url	    = video.getElementsByTagName('url')[0].childNodes[0].data
